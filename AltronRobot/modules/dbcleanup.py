...
import AltronRobot.modules.sql.global_bans_sql as gban_sql
import AltronRobot.modules.sql.users_sql as user_sql
...
def dbcleanup(update: Update, context: CallbackContext):
    ...
    msg.reply_text(f"Getting invalid chat count ...")
    invalid_chat_count = get_invalid_chats(update, context)

    msg.reply_text(f"Getting invalid gbanned count ...")
    invalid_gban_count = get_invalid_gban(update, context)

    reply = f"» ᴛᴏᴛᴀʟ ɪɴᴠᴀʟɪᴅ ᴄʜᴀᴛꜱ - {invalid_chat_count}\n» ᴛᴏᴛᴀʟ ɪɴᴠᴀʟɪᴅ ɢʙᴀɴɴᴇᴅ ᴜꜱᴇʀꜱ - {invalid_gban_count}"
...
def callback_button(update: Update, context: CallbackContext):
    ...
    if query_type == "db_leave_chat":
        ...
        if query.from_user.id in admin_list:
            bot.editMessageText("» ᴄʟᴇᴀɴɪɴɢ ᴜᴘ ᴅʙ ...", chat_id, message.message_id)
            invalid_chat_count = get_invalid_chats(update, context, True)
            invalid_gban_count = get_invalid_gban(update, context, True)
            reply = "» ᴄʟᴇᴀɴᴇᴅ ᴜᴘ {} ᴄʜᴀᴛꜱ ᴀɴᴅ {} ɢʙᴀɴɴᴇᴅ ᴜꜱᴇʀꜱ ꜰʀᴏᴍ ᴅʙ.".format(
