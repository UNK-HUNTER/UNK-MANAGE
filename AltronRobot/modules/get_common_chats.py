...
from AltronRobot import OWNER_ID, dispatcher
...
def get_user_common_chats(update: Update, context: CallbackContext):
    ...
    if not user:
        msg.reply_text("» ɪ ꜱʜᴀʀᴇ ɴᴏ ᴄᴏᴍᴍᴏɴ ᴄʜᴀᴛꜱ ᴡɪᴛʜ ᴛʜᴇ ᴠᴏɪᴅ.")
        return
    common_list = get_user_com_chats(user)
    if not common_list:
        msg.reply_text("» ɴᴏ ᴄᴏᴍᴍᴏɴ ᴄʜᴀᴛꜱ ᴡɪᴛʜ ᴛʜɪꜱ ᴜꜱᴇʀ!")
        return
    name = bot.get_chat(user).first_name
    text = f"<b>» ᴄᴏᴍᴍᴏɴ ᴄʜᴀᴛꜱ ᴡɪᴛʜ {name} :</b>\n\n"
