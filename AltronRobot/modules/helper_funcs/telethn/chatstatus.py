...
from AltronRobot import DRAGONS, telethn
from AltronRobot.modules.helper_funcs.telethn import IMMUNE_USERS
...
async def altron_is_admin(chat_id: int):
    status = False
    altron = await telethn.get_me()
    async for user in telethn.iter_participants(
        chat_id, filter=ChannelParticipantsAdmins
    ):
        if altron.id == user.id:
            status = True
            break
    return status
