...
from AltronRobot import dispatcher, telethn
from AltronRobot.__main__ import (
    CHAT_SETTINGS,
    DATA_EXPORT,
    DATA_IMPORT,
    HELPABLE,
...
def load(update: Update, context: CallbackContext):
    ...
    try:
        imported_module = importlib.import_module("AltronRobot.modules." + text)
    except:
...
def unload(update: Update, context: CallbackContext):
    ...
    unload_messasge = message.reply_text(
        f"» ᴀᴛᴛᴇᴍᴘᴛɪɴɢ ᴛᴏ ᴜɴʟᴏᴀᴅ ᴍᴏᴅᴜʟᴇ : <b>{text}</b>", parse_mode=ParseMode.HTML
    )

    try:
        imported_module = importlib.import_module("AltronRobot.modules." + text)
    except:
        unload_messasge.edit_text("» ᴅᴏᴇꜱ ᴛʜᴀᴛ ᴍᴏᴅᴜʟᴇ ᴇᴠᴇɴ ᴇxɪꜱᴛ?")
        return
    ...
    if imported_module.__mod_name__.lower() in IMPORTED:
        ...
        unload_messasge.edit_text("» ᴄᴀɴ'ᴛ ᴜɴʟᴏᴀᴅ ꜱᴏᴍᴇᴛʜɪɴɢ ᴛʜᴀᴛ ɪꜱɴ'ᴛ ʟᴏᴀᴅᴇᴅ.")
        return
    if "__handlers__" in dir(imported_module):
        handlers = imported_module.__handlers__
        for handler in handlers:
            if isinstance(handler, bool):
                unload_messasge.edit_text("» ᴛʜɪꜱ ᴍᴏᴅᴜʟᴇ ᴄᴀɴ'ᴛ ʙᴇ ᴜɴʟᴏᴀᴅᴇᴅ!")
                return
    ...
    unload_messasge.edit_text(
        f"» ꜱᴜᴄᴄᴇꜱꜱꜰᴜʟʟʏ ᴜɴʟᴏᴀᴅᴇᴅ ᴍᴏᴅᴜʟᴇ : <b>{text}</b>", parse_mode=ParseMode.HTML
    )
...
def listmodules(update: Update, context: CallbackContext):
    message = update.effective_message
    module_list = "» ꜰᴏʟʟᴏᴡɪɴɢ ᴍᴏᴅᴜʟᴇꜱ ᴀʀᴇ ʟᴏᴀᴅᴇᴅ : \n\n"

    for helpable_module in HELPABLE:
        helpable_module_info = IMPORTED[helpable_module]
        file_info = IMPORTED[helpable_module_info.__mod_name__.lower()]
        file_name = file_info.__name__.rsplit("AltronRobot.modules.", 1)[1]
        mod_name = file_info.__mod_name__
        module_list += f"- <code>{mod_name} ({file_name})</code>\n"
