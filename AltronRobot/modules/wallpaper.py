...
from AltronRobot import SUPPORT_CHAT, dispatcher, pbot
...
async def wall(client, message):
    ...
    try:
        ...
        murl = (requests.get(LOGO_API).history[1].url)
        img = Image.open(io.BytesIO(requests.get(LOGO_API).content))
        fname = "AltronRobot.png"
        img.save(fname, "png")
        caption = f"""
